...
public class FresherProcFragment extends Fragment {
	...
	{
		CardView = (CardListView) v.findViewById(R.id.cardsview_fresher);
		CardsAdapter = new CardAdapter<Card>(getActivity().getApplicationContext()).setAccentColorRes(android.R.color.holo_blue_light).setCardsClickable(true).setPopupMenu(-1, null);
		...
		CardsAdapter.add(new CardHeader("For Freshers", "Useful Info for Freshers"));
		getTitles();
	...
	private void getTitles(){
		...
		for (int i = 0; i< Procedures.size(); i++){
			if(Procedures.get(i).getFreshers()){
